const trascender = function(data){
	...
	if(this.scroller!==null){
		$(this.scroller).scroll(()=>{
			if(this.obtained < this.cant && !this.obtaining){
				//if(Math.round($(this.scroller).scrollTop() + $(this.scroller).height()) == Math.round($(document).height())) {
				if($(this.scroller)[0].scrollHeight - $(this.scroller).scrollTop() == parseInt($(this.scroller).height())){
					this.getCollection();
				}
			}
		});
